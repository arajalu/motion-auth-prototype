...
public class TimeSeries
{
   ...
   public TimeSeries(String inputFile, int[] colToInclude, boolean isFirstColTime, boolean isLabeled, char delimiter)
   {
      ...
      try
      {
         ...
         Dir.mkdirs();
         File csvFile = new File(Dir,inputFile);
         Log.d("Debug::",Environment.getExternalStorageDirectory()+"/accData/" + inputFile);
         ...
         else    // time series file is not labeled
         {
            ...
            br.close();
            br = new BufferedReader (new FileReader(csvFile));  // open the input file
         }  // end if
